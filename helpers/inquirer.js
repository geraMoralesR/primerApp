const inquirer = require('inquirer');
const Tarea = require('../model/tarea');

require('colors');

const menuOpts = [{
    type: 'list',
    name: 'options',
    message: `${'Que decea hacer?'.brightYellow}`,
    choices: [
        {
            value: '1',
            name: `${'1.-'.cyan} Crear tarea`
        },
        {
            value: '2',
            name: `${'2.-'.cyan} listar tarea`
        },
        {
            value: '3',
            name: `${'3.-'.cyan} listar tareas completadas`
        },
        {
            value: '4',
            name: `${'4.-'.cyan} Listar tareas pendientes`
        },
        {
            value: '5',
            name: `${'5.-'.cyan} Completar tarea`
        },
        {
            value: '6',
            name: `${'6.-'.cyan} Borrar tareas`
        },
        {
            value: '0',
            name: `${'0.-'.cyan} Salir`
        }
    ]
}];


const inquirerMenu = async () => {
    console.clear();
    console.log("===========================".green);
    console.log("   Seleccione una opción   ".white);
    console.log("===========================\n".green);

    const { options } = await inquirer.prompt(menuOpts);

    return options;

}

const pausa = async () => {
    const question = [
        {
            type: 'input',
            name: 'enter',
            message: `Presione ${'Enter'.green} para continuar`
        }
    ];

    console.log('\n');
    await inquirer.prompt(question);
}

const leerInput = async (message) => {
    const question = [
        {
            type: 'input',
            name: 'desc',
            message,
            validate(value) {
                if (value.length === 0) {
                    return ' ' + 'Ingrese una tarea';
                }
                return true;
            }
        }
    ];
    const { desc } = await inquirer.prompt(question);
    return desc;
}

const listadoBorrarTarea = async (tareas = []) => {
    const choices = tareas.map((tarea, i) => {
        const idx = `${i + 1}.`.green;

        return {
            value: tarea.id,
            name: `${idx} ${tarea.desc}`
        }
    });

    choices.unshift({
        value: '0',
        name:'0.'.green + ' ' + 'Cancelar operación'.yellow
    });

    const preguntas = [
        {
            type: 'list',
            name: 'id',
            message: 'borrar',
            choices
        }
    ]
    const { id } = await inquirer.prompt(preguntas);
    return id;
}

const comfirmar = async (message) => {
    const question = [
        {
            type: 'comfirm',
            name: 'ok',
            message
        }
    ];
    const { ok } = await inquirer.prompt(question);
    return ok;
}


const listadoPendientes = async (tareas = []) => {
    const choices = tareas.map((tarea, i) => {
        const idx = `${i + 1}.`.green;

        return {
            value: tarea.id,
            name: `${idx} ${tarea.desc}`,
            checked: (tarea.completadoEn) ? true : false
        }
    });

    const pregunta = [
        {
            type: 'checkbox',
            name: 'ids',
            message: 'selecciones',
            choices
        }
    ]
    const { ids } = await inquirer.prompt(pregunta);
    return ids;
}


module.exports = {
    inquirerMenu,
    pausa,
    leerInput,
    listadoBorrarTarea,
    comfirmar,
    listadoPendientes
}